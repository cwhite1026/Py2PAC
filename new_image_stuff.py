...
flag_map_filename = '/user/mdurbin/candels/flag_maps/flag_map_s_f160w.fits'
flag_map = fits.open(flag_map_filename)
flag_array=np.asarray(flag_map[0].data)
binary_flag = np.where(flag_array==0, 0, 1)
flag_slices = meas.find_objects(binary_flag)[0]
binary_flag = binary_flag[flag_slices]
image_dims = np.array(binary_flag.shape)
binary_image_with_border = np.zeros(image_dims+2)
binary_image_with_border[1:-1, 1:-1] = binary_flag
